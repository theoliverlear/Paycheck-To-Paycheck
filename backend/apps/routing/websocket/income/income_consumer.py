...
from backend.apps.comm.serialize.entity.income.income_serializer import \
    IncomeSerializer
...
class IncomeConsumer(WebSocketConsumer[Income]):
    async def receive(self, text_data=None, bytes_data=None) -> None:
        if text_data:
            data = json.loads(text_data)
            income: Income = self.get_income(data)
